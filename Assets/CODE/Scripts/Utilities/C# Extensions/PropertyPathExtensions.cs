...
public static class PropertyPathExtensions
{
    ...
    {
        if (string.IsNullOrWhiteSpace(pathString)) throw new ArgumentException("Path string is null or empty.");
        ...
        foreach (var part in pathString.Split('.'))
        {
            ...
            {
                if (!int.TryParse(part[(bracketStart + 1)..bracketEnd], out var index)) throw new FormatException($"Invalid index in path: {part[(bracketStart + 1)..bracketEnd]}");
            ...
            if (bracketStart >= 0) throw new FormatException($"Unmatched '[' in path: {part}");
        }
